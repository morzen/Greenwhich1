...
pizzas = ["margherita", "napoletana", "marinara", "calzone"]
pizza_prices = [6.0, 7.0, 7.5, 8.0]
for i in range(len(pizzas)):
    rad = Radiobutton(window, text=pizzas[i] + " - £" + format(pizza_prices[i], "4.2f"), value=i, variable=selected, command=clicked)
...
extras = ["mushrooms", "cheese", "anchovies", "sausage", "garlic"]
extra_prices = [0.5, 1.0, 1.5, 1.8, 0.70]
extra_states = []
for i in range(len(extras)):
    extra_states.append(BooleanVar())
    chk = Checkbutton(window, text=extras[i] + " - £" + format(extra_prices[i], "4.2f"), var=extra_states[i], command=clicked)
